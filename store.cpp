...
public:
    ...
    // BENCHMARK ALGORITHM: Linear Search (O(n))
    bool linearSearchByID(int targetID)
    {
        DeliveryNode *temp = deliveryHead;
        while (temp != NULL)
        {
            if (temp->orderInfo.orderID == targetID)
            {
                return true;
            }
            temp = temp->nextNode;
...
void loadBenchmarks(string filename)
{
    ...
    while (getline(file, line))
    {
            ...
            getline(ss, addr, ','))
        {
            ...
            // Insert into List (O(1) insertion, O(n) search)
...
void runPerformanceTest()
{
    ...
    int targetID = globalNextOrderId - 1; // Worst case for Linked List
    cout << endl << "===== BENCHMARK RESULTS (N=500) =====" << endl;
    cout << "Searching for ID: " << targetID << endl;
    cout << "Running 10,000 iterations for accuracy." << endl;
    ...
    for (int i = 0; i < 10000; i++)
    {
        storePtr->linearSearchByID(targetID);
    }
    clock_t endList = clock();
    double timeList = double(endList - startList) / CLOCKS_PER_SEC * 1000;

    cout << endl << "-----------------------------------------------------" << endl;
    cout << "ALGORITHM\t\t| TIME (ms)\t| COMPLEXITY" << endl;
    cout << "-----------------------------------------------------" << endl;
    cout << "AVL Tree Search\t\t| " << timeAVL << " ms\t| O(log n)" << endl;
    cout << "Linked List Search\t| " << timeList << " ms\t| O(n)" << endl;
    cout << "-----------------------------------------------------" << endl;
    cout << "VERDICT: AVL Tree is significantly faster for searching." << endl;
}
